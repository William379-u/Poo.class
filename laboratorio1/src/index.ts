...
class estudiante{
   ...
   public correo: string;
    private codigoIndentificacion: number;

    constructor(nombre: string, apellido: string, telefono: number, correo: string, codigoIndentificacion: number){
    this.nombre = nombre;
...
class producto{
    ...
    constructor(cantidad_de_producto: number, precio: number, descuento: number, iva: number, total_a_pagar: number){
...
class Persona{
    ...
    constructor(Nombre: string, Fecha_de_nacimiento: Date){
        this.Nombre = Nombre;
...
function determinarEstado(edad: number): string {
    if (edad >= 0 && edad <= 2) {
        return "Bebé";
