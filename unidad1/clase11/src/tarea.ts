...
class Trabajador {
  ...
  pedirDatos() {
    this.rl.question("Ingrese el usuario: ", (user: string) => {
        this.rl.question('Ingrese la contraseña: ', (password: string) => {
            this.rl.question('Ingrese el número que corresponda a su rol (1 = Administrador, 2 = Cliente, 3 = Inivitado): ', (rol : string) => {
  ...
  validarDatos(user: string, password: string, rol: number) {
    if (user !== 'Chepe') {
